...
(async () => {
  ...
  // HTTP authentication credentials (Replace with your actual credentials)
  const authCredentials = {
    username: 'gsdmafrs', // Update with actual username
    password: 'aimhigh!1'  // Update with actual password
  };
  ...
  const browser = await puppeteer.launch({
    ...
    protocolTimeout: 120000 // Increased timeout for heavy pages
  });
  ...
  for (const url of urlsToTest) {
    ...
    for (const viewport of viewports) {
      ...
      try {
        ...
        // Scroll and wait to ensure all elements load (handles lazy-loaded content)
        ...
        const results = await page.evaluate(async () => {
          ...
          // Check all iframes (if any exist)
  ...
  await browser.close(); // ✅ Ensures Puppeteer closes properly
})();
